...
function init() {
    ...
    finalScoreScreen = document.getElementById('finalscore');
    helpmenu = document.getElementById('helpmenu');


    //gets the properties of the window - width and height - directly from the CSS
    screenWidth = parseFloat(getComputedStyle(screen).getPropertyValue('width'));
    screenHeight = parseFloat(getComputedStyle(screen).getPropertyValue('height'));


    //gets the ghost width/height and border values, so the ghost doesn't overflow off the screen
    ghostWidth = parseFloat(getComputedStyle(ghost).getPropertyValue('width'));
    ghostHeight = parseFloat(getComputedStyle(ghost).getPropertyValue('height'));
    screenBorder = parseFloat(getComputedStyle(screen).getPropertyValue('border-width'));
...
function start(){
            ...
            setTimeout(stop,  59 * 1000); //this makes the game end - at 1 minute
}
